...
import StringIO
import uuid
from dime import Message, Record, TypeByUri, MediaType
...
class SoapClient:
    ...
    def invoke(self, args, kwargs):
        ...
        if attachments:
            result = self.send_dime(msg, attachments)
        else:
            result = self.send_mime(msg)
        timer.stop()
        metrics.log.debug(
                "method '%s' invoked: %s",
                self.method.name, timer)
    ...
    def send_dime(self, msg, attachments):
        def dime_headers():
            """
            ...
            stock = { 'Content-Type' : 'application/dime', 
                     'SOAPAction': action }
        ...
        try:
            ...
            # FIXME: Now can upload only one file
            filename = attachments[0]
            records = []
            out = StringIO.StringIO()
            id =  uuid.uuid1()
            records.append(Record(str(id),
                            TypeByUri('http://schemas.xmlsoap.org/soap/envelope'),
                            str(msg),
                            0x01,
                            0x00,
                            0x00,
